...
![Prerequisites: M12 — SQLite](https://img.shields.io/badge/Prerequisites-M12:_SQLite-94a3b8?style=flat-square)
...
**Scenario:** A small café manages its drink menu in three places: the website, a printed handout, and a Slack bot for staff. Every time a price changes, all three need to be updated separately. We will build a **mini coffee-shop menu API** so any client pulls from a single source — update the code once, everywhere stays in sync.
...
* [ ] **AI Polish Service**
  ...
  - `POST /polish` accepts body `{"text": "rough draft here"}`. Calls local Ollama, returns `{"polished": "...", "elapsed_seconds": 3.2}`.
